 ...
 */
public class Right_Menu_Crime_Tracking extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SpannableStringBuilder SS = new SpannableStringBuilder(Html.fromHtml("<font color='#3c5899'><small><b>CRIME TRACKING</b></small> </font>"));
        SS.setSpan(new CustomTypefaceSpan("#3c5899", tf), 0, SS.length(), Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
        getSupportActionBar().setTitle(SS);

        //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> A C T I O N B A R >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        webView = (WebView) findViewById(R.id.webView1);
        startWebView(CONFIG.URL+"/staticpages/crime_tracking.html");
        ...
        webView.setWebViewClient(new WebViewClient() {
            ...
            public void onLoadResource(WebView view, String url) {
                if (progressDialog == null) {
                    ...
                    progressDialog = new ProgressDialog(Right_Menu_Crime_Tracking.this);
