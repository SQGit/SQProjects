...
//        canvas.drawPaint(mPaint);
    ...
    private float getOriginalWidth() {
        if (!isInEditMode()) {
            ...
            return (getOrientation() == HORIZONTAL) ? 0 : 0;
        }
    }
    ...
    private RectF getRect(float w, float h) {
        ...
        if (availableWidth / availableHeight >= getOriginalWidth() / getOriginalHeight()) {
                ...
                float x = getPaddingLeft() + (availableWidth - alpha*getOriginalWidth()) / 1.0f;
            ...
            float y = getPaddingTop() + (availableHeight - alpha*getOriginalHeight()) / 1.0f;
